...
class ChaCha20Poly1305:
    ...
    def decrypt(self, ciphertext: bytes, seq_num: int):
    ...
    def decrypt_size(self,ciphertext: bytes,seq_num: int):
        nonce = ZERO  + int.to_bytes(seq_num,8) # nonce for size
        return self.do_decrypt(self.key1,nonce,ciphertext)
    
    def do_encrypt(self,key,nonce,plaintext):
        cipher = Cipher(ChaCha20(key, nonce), None)
        return cipher.encryptor().update(plaintext)
    
    def do_decrypt(self,key,nonce,ciphertext):
        cipher = Cipher(ChaCha20(key, nonce), None)
        return cipher.decryptor().update(ciphertext)
    
    def verify(self, ciphertext: bytes, mac: bytes,seq_num: int):
    ...
    def digest(self, ciphertext: bytes,seq_num: int):
        key = self.generate_mac_key(seq_num)
        return Poly1305.generate_tag(key, ciphertext)
    
    def generate_mac_key(self,seq_num: int):
