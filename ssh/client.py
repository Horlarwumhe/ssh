...
class SSHClient:
    ...
    async def connect(self,host,port,start_kex=False):
        ...
        while True:
            ...
            if line.startswith(b'SSH-'):
                ...
                self._log(logging.INFO,' setting remote_version %s'%self.remote_version)
                break
    ...
    async def start_kex(self):
        req = msg.SSHMsgKexInit(
                ...
                encryption_algo_client_to_server=list(self.available_encryption_algo.keys()),
                encryption_algo_server_to_client=list(self.available_encryption_algo.keys()),
    ...
    def set_algos(self,server_kex: msg.SSHMsgKexInit):
        self.kex_algo = list(filter(lambda x: x in server_kex.kex_algo,self.available_kex_algo))[0]
        self.server_host_key_algo = list(filter(lambda x: x in server_kex.server_host_key_algo,self.available_server_host_key_algo))[0]
        self.encryption_algo = list(filter(lambda x: x in server_kex.encryption_algo_client_to_server,self.available_encryption_algo))[0]
        self.mac_algo = list(filter(lambda x: x in server_kex.mac_algo_client_to_server,self.available_mac_algo))[0]
        self.compression_algo = "none" #TODO
        self.logger.log(logging.DEBUG,"Kex Algo: %s",self.kex_algo)
        self.logger.log(logging.DEBUG,"server host key algo: %s",self.server_host_key_algo)
    ...
    def set_ciphers(self,K,H):
        ...
        if mac_algo is not None:
            ...
            self.sock.set_mac_algo(mac_algo(client_to_server_mac),mac_algo(server_to_client_mac))
        ...
        # self.sock.set_mac_algo(self.available_mac_algo[self.mac_algo])
        # self.sock.set_keys( )
        # print(
        #     f"{client_to_server_iv.hex()=}\n"\
        #     f"{client_to_server_key.hex()=}\n"\
        #     f"{server_to_client_iv.hex()=}\n"\
        #     f"{server_to_client_key.hex()=}\n"\
        #     f"{server_to_client_mac.hex()=}\n"\
        #     f"{client_to_server_mac.hex()=}\n"\
        # )
            # import pd`b
            # pdb.set_trace()`
        assert len(client_to_server_iv) == iv_size, "(iv size) %s != cipher iv_size (%s)"%(len(client_to_server_iv),iv_size)
        assert len(client_to_server_key) == key_size, "(key size) %s != cipher key_size (%s)"%(len(client_to_server_iv),key_size)
        if mac_algo:
            assert len(client_to_server_mac) == mac_size, "(mac key size) %s != mac key_size (%s)"%(len(client_to_server_mac),mac_size)
