...
class SSHMessage:
    ...
    def validate(cls, m: Buffer):
        ...
        assert opcode == cls.opcode, "Opcode mismatch cls.opcode != opcode %s != %s" % (cls.opcode, opcode)
...
class SSHMessageKexDHReply(SSHMessage):
    ...
    def parse(cls,m:Buffer):
        cls.validate(m)
        return cls(
                    host_pub_key_cert=m.read_binary(),       
                    f=m.read_mpint(),
                    sig=m.read_binary())
...
class SSHKexECDHInit(SSHMessage):
    ...
    def parse(cls, buf:Buffer):
        cls.validate(buf)
        return cls(
            client_pub_key=buf.read_binary()
        )
    def __bytes__(self):
        buf = Buffer()
...
class SSHKexECDHReply(SSHMessage):
    ...
    def parse(cls, buf):
        cls.validate(buf)
        return cls(
            host_key=buf.read_binary(),
            pub_key = buf.read_binary(),
            sig = buf.read_binary()
        )
    def __bytes__(self):
        buf = Buffer()
