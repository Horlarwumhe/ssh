...
class Packet:
    ...
    def __init__(
        self, data: bytes | None = None, payload: bytes | None = None,packet_length: int = 0, padding_length: int = 0,
    ):
    ...
    def build(cls, payload: bytes,block_size = 8) -> "Packet":
        ...
        return Packet(
            data=buf.getvalue(), packet_length=packet_length, padding_length=padding_length
        )
...
class Connection:
    ...
    async def read_packet(self):
        ...
        assert (4 + 1 + len(payload) + len(padding)) % self.block_size == 0, " block_size != 0"
        p = Packet(payload=payload)
        log.log(logging.DEBUG,f"[incoming packet_length={size} opcode={code_to_desc(p.opcode)}]")
    ...
    async def read_encrypted_packet(self):
        ...
        my_mac = self.server_mac.digest(struct.pack(">IIB",self.server_seq_no,size,padding_length)+payload+padding)
        assert mac == my_mac, "error computed mac not macth server mac mac(%s) != my_mac(%s) "%(mac.hex(),my_mac.hex())
    ...
    async def send_packet(self,data: bytes):
        ...
        data = bytes(p)
        assert p.packet_length + 4 == len(data), "%s != %s"%(p.packet_length+4,len(data))
        log.log(logging.DEBUG,f"[outgoing] {cmd=} {p.packet_length=} {len(data)=} {p.padding_length=}")
        mac = b''
        if self.encrypted:
