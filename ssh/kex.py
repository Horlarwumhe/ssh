...
from .hash import sha1, sha256, sha512, sha384
...
class ECDH(Kex):
    ...
    def generate_key_pair(self):
        ...
        pass
    def compute_shared_secret(self, pk, server_pub_key: bytes) -> int:
        # implemented in subclasses
        pass
...
class ECDHNISTP(ECDH):
    # ecdh-sha2-nistp*
    def generate_key_pair(self):
        pk = ec.generate_private_key(self.curve)
        pub = pk.public_key()
        return pk, pub.public_bytes(
                        encoding=serialization.Encoding.X962,
                        format=serialization.PublicFormat.UncompressedPoint
                        )
    
    def compute_shared_secret(self, pk: ec.EllipticCurvePrivateKey, server_pub_key: bytes) -> int:
        server_pub = ec.EllipticCurvePublicKey.from_encoded_point(self.curve, server_pub_key)
        return int.from_bytes(pk.exchange(ec.ECDH(), server_pub))
    
class ECDHNISTP256(ECDHNISTP):
    name = "ecdh-sha2-nistp256"
    curve = ec.SECP256R1()
    hash_algo = staticmethod(sha256)

class ECDHNISTP384(ECDHNISTP):
    name = "ecdh-sha2-nistp384"
    curve = ec.SECP384R1()
    hash_algo = staticmethod(sha384)

class ECDHNISTP521(ECDHNISTP):
